...
function AddGoodsModal({ onClose, onSuccess }) {
  const [form, setForm] = useState({ date: TODAY, metalType: "", metalSize: "", perKgRate: "", totalKg: "" });
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <div className="relative">
                ...
                {/* ✅ FIX: type="text" inputMode="decimal" — 300 → 175 wala bug fix */}
          ...
          {items.map((g, i) => {
            ...
            return (
              <motion.div key={g.id || i}
                ...
                className="rounded-2xl p-4" style={{ background: m.dim, border: `1px solid ${m.color}20` }}>
...
export default function GoodsInStock() {
  ...
  const grouped = goods.reduce((acc, g) => {
    ...
    acc[date].push({ id: g._id, metalType: g.materialType, metalSize: g.size, perKgRate: g.perKgRate, totalKg: g.totalKg, amount: g.totalAmount });
    return acc;
  }, {});
  ...
  const filtered = dateSummaries.filter(d =>
    d.date.includes(search) || (search === "ms" && d.ms > 0) || (search === "gi" && d.gi > 0)
  );

  const totalPages = Math.ceil(filtered.length / ITEMS_PER_PAGE);
  const currentDates = filtered.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);

  const handleAddSuccess = () => { addToast("Goods successfully add ho gaya!", "success"); fetchAll(); };
  ...
  return (
    <div className="px-4 sm:px-5 lg:px-6 pb-6 pt-1" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <>
          {currentDates.length === 0 ? (
            ...
            <div className="rounded-2xl overflow-hidden" style={{ background: "#0c0e1a", border: "1px solid #1e2235" }}>
              ...
              <div className="p-3 md:p-2 md:px-3 space-y-0">
                <AnimatePresence>
                  {currentDates.map((d, i) => <DateRow key={d.date} d={d} index={i} onView={setViewDate} />)}
                </AnimatePresence>
              </div>
            </div>
          )}
          ...
          {totalPages > 1 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
              ...
              <motion.button whileTap={{ scale: 0.9 }} onClick={() => setCurrentPage(p => Math.max(p - 1, 1))} disabled={currentPage === 1}
                className="px-4 py-2 rounded-xl text-sm font-bold transition-all disabled:opacity-30"
                style={{ background: "#0c0e1a", border: "1px solid #1e2235", color: "#6b7a99" }}>← Prev</motion.button>
              <div className="flex gap-1">
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                  <motion.button key={p} whileTap={{ scale: 0.88 }} onClick={() => setCurrentPage(p)}
                    className="w-9 h-9 rounded-xl text-sm font-black transition-all"
                    style={{ background: currentPage === p ? "#3b82f6" : "#0c0e1a", border: `1px solid ${currentPage === p ? "#3b82f6" : "#1e2235"}`, color: currentPage === p ? "#fff" : "#4a5580", boxShadow: currentPage === p ? "0 4px 12px #3b82f630" : "none" }}>
                    {p}
                  </motion.button>
                ))}
              </div>
              <motion.button whileTap={{ scale: 0.9 }} onClick={() => setCurrentPage(p => Math.min(p + 1, totalPages))} disabled={currentPage === totalPages}
                className="px-4 py-2 rounded-xl text-sm font-bold transition-all disabled:opacity-30"
                style={{ background: "#0c0e1a", border: "1px solid #1e2235", color: "#6b7a99" }}>Next →</motion.button>
            </motion.div>
          )}
        </>
      )}
      ...
      <AnimatePresence>
        {showAdd && <AddGoodsModal onClose={() => setShowAdd(false)} onSuccess={handleAddSuccess} />}
      </AnimatePresence>
      <AnimatePresence>
        {viewDate && grouped[viewDate] && <ViewModal date={viewDate} items={grouped[viewDate]} onClose={() => setViewDate(null)} />}
      </AnimatePresence>
    </div>
  );
}