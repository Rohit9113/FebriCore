...
// ══════════════════════════════════════════════════════
// 🔥 WELD SPARK — sparks fly on field focus
// ══════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════
// 🔥 FORGE FIELD — input with weld spark on focus
// ══════════════════════════════════════════════════════
function ForgeField({ label, icon, children }) {
  ...
  return (
    <div>
      <motion.p
        animate={{ color: focused ? "#f59e0b" : "#3d4260" }}
        transition={{ duration: 0.2 }}
        className="text-[10px] font-black uppercase tracking-[0.18em] mb-2"
      >
        {icon && <span className="mr-1.5">{icon}</span>}{label}
      </motion.p>
      <div className="relative" onFocus={handleFocus} onBlur={() => setFocused(false)}>
        {/* Heat glow border */}
        <motion.div
          animate={{ opacity: focused ? 1 : 0, scale: focused ? 1 : 0.98 }}
          transition={{ duration: 0.2 }}
          className="absolute -inset-px rounded-2xl pointer-events-none"
          style={{ background: "linear-gradient(135deg,#f59e0b22,transparent,#f97316/18)", border: "1px solid #f59e0b44" }}
        />
        {children}
...
// ══════════════════════════════════════════════════════
// 🔥 3D MAGNETIC CARD — tilts toward cursor
// ══════════════════════════════════════════════════════
function MagneticCard({ children, className, onClick, selected }) {
  ...
  const handleLeave = useCallback(() => {
    rotX.set(0); rotY.set(0); gX.set(50); gY.set(50);
  }, [rotX, rotY, gX, gY]);

  const background = useTransform([gX, gY], ([x, y]) =>
    `radial-gradient(circle at ${x}% ${y}%, ${selected ? "#f59e0b18" : "#ffffff08"} 0%, transparent 65%)`
  );
  ...
  return (
    ...
    >
      <motion.div style={{ background }} className="absolute inset-0 rounded-2xl pointer-events-none z-10" />
      {children}
    </motion.div>
  );
}
...
// ══════════════════════════════════════════════════════
// 🔥 STEP CONNECTOR — SVG line draws itself
// ══════════════════════════════════════════════════════
function StepLine({ filled }) {
  return (
    <div className="flex-1 relative h-px mx-1">
      ...
      />
      {/* Spark traveling along line */}
      {filled && (
        <motion.div
          initial={{ left: 0, opacity: 0 }}
          animate={{ left: "100%", opacity: [0, 1, 0] }}
          transition={{ duration: 0.5, ease: "easeOut" }}
          className="absolute top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-amber-400 shadow-lg shadow-amber-400/60"
...
// ══════════════════════════════════════════════════════
// 🔥 STEP PILL — animated number/check badge
// ══════════════════════════════════════════════════════
function StepPill({ number, label, state, onClick }) {
  return (
    <motion.button onClick={onClick} className="flex items-center gap-2 cursor-pointer" whileTap={{ scale: 0.92 }}>
      <div className="relative w-8 h-8">
        ...
        {state === "active" && (
          <motion.div
            ...
            className="absolute inset-0 rounded-xl bg-amber-500/40"
          />
        )}
        <motion.div
          layout
          animate={{
            background: state === "done" ? "linear-gradient(135deg,#10b98122,#059669/10)" :
              state === "active" ? "linear-gradient(135deg,#f59e0b,#f97316)" : "#0c0e1a",
            borderColor: state === "done" ? "#10b981/50" : state === "active" ? "#f59e0b" : "#1e2235",
      ...
      <span className={`text-xs font-black hidden sm:block tracking-wider uppercase transition-colors duration-300
        ${state === "done" ? "text-emerald-400" : state === "active" ? "text-amber-300" : "text-[#2e3248]"}`}>
        {label}
      </span>
    </motion.button>
  );
...
function MetalPill({ label, selected, onClick }) {
  return (
    <motion.button
      ...
      className={`relative px-4 py-2.5 rounded-xl text-sm font-black border overflow-hidden transition-colors
        ${selected
          ? "bg-amber-500/15 border-amber-500/50 text-amber-300"
          : "bg-[#0c0e1a] border-[#1e2235] text-[#3d4260] hover:border-[#2e3248]"}`}
    >
      {selected && (
        <>
          <motion.div
          ...
          <motion.div
            ...
            className="absolute inset-0 bg-gradient-to-r from-transparent via-amber-400/15 to-transparent skew-x-12"
          />
        </>
      )}
...
// ══════════════════════════════════════════════════════
// 🔥 ORDER TYPE CARD — 3D magnetic + animated stripe
// ══════════════════════════════════════════════════════
const TYPE_META = {
...
function OrderTypeCard({ type, selected, onClick }) {
  ...
  return (
    ...
    >
      <motion.div
        animate={{
          borderColor: selected ? `${m.color}55` : "#1e2235",
          background: selected ? `${m.color}08` : "#0c0e1a",
          boxShadow: selected ? `0 4px 30px ${m.color}15, inset 0 1px 0 ${m.color}22` : "none",
        }}
        transition={{ duration: 0.25 }}
        className="relative w-full text-left rounded-2xl border p-4 overflow-hidden z-0"
      >
        {/* Animated left bar */}
        <motion.div
          animate={{ scaleY: selected ? 1 : 0, opacity: selected ? 1 : 0 }}
          style={{ transformOrigin: "bottom", background: m.color }}
        ...
        <div className="flex items-center gap-3 pl-3">
          <motion.span
            animate={{ scale: selected ? 1.3 : 1, rotate: selected ? [0, -12, 0] : 0 }}
...
// ══════════════════════════════════════════════════════
// 🔥 SHIMMER BUTTON — glowing CTA with light sweep
// ══════════════════════════════════════════════════════
function ShimmerBtn({ active, onClick, children, className = "", green = false }) {
  ...
  const inactive = "bg-[#0c0e1a] border border-[#1e2235] text-[#2e3248]";
  ...
  return (
    <motion.button
      ...
      className={`relative overflow-hidden font-black transition-all duration-300 ${active ? base : inactive} ${className}`}
    >
      {active && (
        <motion.div
          animate={{ x: ["-120%", "220%"] }}
      ...
      {active && !green && (
        <motion.div
          ...
          className="absolute inset-0 bg-amber-300/20 pointer-events-none rounded-inherit"
        />
      )}
...
// ══════════════════════════════════════════════════════
// 🔥 SUCCESS SCREEN — molten sparks explosion
// ══════════════════════════════════════════════════════
...
function SuccessScreen({ customerName, orderCount, orderType, onReset }) {
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="fixed inset-0 z-50 bg-[#06080f] flex flex-col items-center justify-center overflow-hidden"
    ...
    >
      ...
      {/* Spark burst — positioned at center */}
        ...
        {sparks.map(s => {
          ...
          return (
            <motion.div key={s.id}
              ...
              transition={{ delay: s.delay, duration: s.dur, ease: [0.15, 0.8, 0.4, 1] }}
              className="absolute pointer-events-none"
              style={{
                width: s.size, height: s.size * (s.trailLen ? 3 : 1),
                borderRadius: s.trailLen ? "2px" : "50%",
                background: s.color,
                boxShadow: `0 0 ${s.size * 3}px ${s.color}`,
                transform: s.trailLen ? `rotate(${s.angle}deg)` : undefined,
              }}
            />
          );
        })}
      ...
      >
        <div className="w-28 h-28 rounded-3xl flex items-center justify-center"
          style={{ background: "linear-gradient(135deg,#10b98120,#059669/10)", border: "2px solid #10b98140" }}>
          <svg width="56" height="56" viewBox="0 0 56 56" fill="none">
            <motion.path
              d="M12 28 L23 39 L44 17"
      ...
      <motion.h2
        ...
        className="text-5xl font-black text-white mb-3 relative z-10"
      ...
      <motion.div
        ...
        className="relative z-10 text-center mb-10 space-y-1"
      >
        <p className="text-[#4a5580] text-base">{customerName} ka order ban gaya</p>
        <p className="text-amber-400 font-black text-xl">
          {orderCount} item{orderCount > 1 ? "s" : ""} <span className="text-[#2e3248]">·</span> {orderType}
        </p>
      </motion.div>
      ...
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.88, type: "spring", stiffness: 250, damping: 22 }}
        className="relative z-10 w-full max-w-xs px-6"
      >
        <ShimmerBtn active onClick={onReset} className="w-full h-14 rounded-2xl text-base">
          + Naya Order Banao
        </ShimmerBtn>
      </motion.div>
    </motion.div>
...
// ══════════════════════════════════════════════════════
// 🔥 CUSTOMER SUMMARY CARD — for sidebar
// ══════════════════════════════════════════════════════
function CustomerCard({ customer, orderCount, onEdit }) {
  return (
    <motion.div
      ...
      style={{
        background: "linear-gradient(145deg,#0e1020 0%,#0c0e1a 60%,#080a14 100%)",
        border: "1px solid #1e2235",
        boxShadow: "inset 0 1px 0 #f59e0b15",
      }}
    >
      <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-amber-500/35 to-transparent" />
      <div className="absolute top-0 right-0 w-32 h-32 bg-amber-500/6 rounded-full blur-2xl pointer-events-none" />
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-white font-black text-lg leading-tight truncate"
              style={{ fontFamily: "'Syne', sans-serif" }}>{customer.customerName}</p>
          ...
          <motion.button
            whileTap={{ scale: 0.88 }}
            onClick={onEdit}
            className="w-8 h-8 rounded-xl bg-[#1e2235] border border-[#2e3248] text-[#4a5580] hover:text-amber-400 hover:border-amber-500/30 transition-all flex items-center justify-center text-sm flex-shrink-0"
          >✏️</motion.button>
        </div>

        {/* Badges */}
        <div className="flex flex-wrap gap-2 mb-4">
          <span className="text-[10px] px-2.5 py-1.5 rounded-xl font-black uppercase tracking-wider"
            style={{ background: "#f59e0b15", border: "1px solid #f59e0b30", color: "#f59e0b" }}>
            {customer.orderType}
          </span>
          <span className="text-[10px] px-2.5 py-1.5 rounded-xl font-black"
            style={{ background: "#1e2235", border: "1px solid #2e3248", color: "#4a5580" }}>
...
// ══════════════════════════════════════════════════════
// 🔥 ORDER CARD — hammer strike entry + delete
// ══════════════════════════════════════════════════════
function OrderCard({ order, index, orderType, onChange, onRemove }) {
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0, scale: 1 }}
      exit={{ opacity: 0, x: -40, scale: 0.88, transition: { duration: 0.2 } }}
    ...
    >
      ...
      <div className="flex items-center gap-3 px-5 py-4"
        ...
        <motion.button
          ...
          style={{ background: "#1a0c0c", border: "1px solid #ef444425", color: "#ef4444" }}
        >🗑️</motion.button>
      </div>
        ...
        {orderType === "Normal Order" ? (
          <>
            <ForgeField label="Item Name" icon="🔧">
              <input value={order.itemName} onChange={e => onChange("itemName", e.target.value)}
                placeholder="Grill, Gate, Door..." className={inp} />
            </ForgeField>
            ...
            <div>
              ...
              <div className="flex gap-2 flex-wrap">
                {METAL_TYPES.map(m => (
                  <MetalPill key={m} label={m} selected={order.metalType === m} onClick={() => onChange("metalType", m)} />
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <ForgeField label="Height (ft)" icon="↕️">
                <input type="number" inputMode="decimal" value={order.height}
                  onChange={e => onChange("height", e.target.value)} placeholder="42" className={inp} />
              </ForgeField>
              <ForgeField label="Width (ft)" icon="↔️">
                <input type="number" inputMode="decimal" value={order.width}
                  onChange={e => onChange("width", e.target.value)} placeholder="26" className={inp} />
              </ForgeField>
            </div>

            <div className="grid grid-cols-2 gap-3">
              <ForgeField label="Rate/kg (₹)" icon="💲">
                <input type="number" inputMode="decimal" value={order.perKgRate}
                  onChange={e => onChange("perKgRate", e.target.value)} placeholder="90" className={inp} />
              </ForgeField>
              <ForgeField label="Extra Charge" icon="➕">
                <input type="number" inputMode="decimal" value={order.extraCharge}
                  onChange={e => onChange("extraCharge", e.target.value)} placeholder="0" className={inp} />
              </ForgeField>
            </div>

            <ForgeField label="Description" icon="📝">
              <input value={order.description} onChange={e => onChange("description", e.target.value)}
                placeholder="Blue sheet, extra fitting..." className={inp} />
            </ForgeField>
          </>
        ) : (
          <>
            <ForgeField label="Amount (₹)" icon="💰">
              <div className="relative">
                <motion.span
                  animate={{ color: order.amount ? "#f59e0b" : "#2e3248", scale: order.amount ? 1.1 : 1 }}
                  transition={{ type: "spring", stiffness: 400, damping: 20 }}
                  className="absolute left-4 top-1/2 -translate-y-1/2 font-black text-lg pointer-events-none"
                >₹</motion.span>
                <input type="number" inputMode="decimal" value={order.amount}
                  onChange={e => onChange("amount", e.target.value)} placeholder="5000" className={`${inp} pl-9`} />
              </div>
            </ForgeField>
            <ForgeField label="Description" icon="📝">
              <textarea rows={3} value={order.description} onChange={e => onChange("description", e.target.value)}
                placeholder="Kaam ka description..." className={`${inp} resize-none`} />
            </ForgeField>
          </>
        )}
      </div>
...
export default function OrdersSection() {
  ...
  const remOrder = (i) => {
    if (orders.length === 1) return showToast("Kam se kam ek order chahiye", "error");
    setOrders(p => p.filter((_, idx) => idx !== i));
  };

  const canS2 = !!(customer.orderType && customer.customerName && customer.phone);
  const canS3 = orders.length > 0 && orders.every(o =>
    customer.orderType === "Normal Order" ? o.itemName : (o.description || o.amount)
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showToast(err?.response?.data?.error || "Order create nahi hua", "error");
    } finally { setLoading(false); }
  };

  const reset = () => {
    setCustomer({ orderType:"", date:TODAY, customerName:"", phone:"", address:"" });
    ...
    <div className="min-h-screen bg-[#06080f]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[900px] h-[500px] rounded-full blur-[160px]"
          style={{ background: "radial-gradient(ellipse,#f59e0b06 0%,transparent 70%)" }} />
        <div className="absolute -bottom-40 right-0 w-[500px] h-[500px] rounded-full blur-[140px]"
          style={{ background: "radial-gradient(ellipse,#3b82f608 0%,transparent 70%)" }} />
      </div>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            className={`fixed top-20 left-4 right-4 lg:left-auto lg:right-6 lg:w-96 z-[200] flex items-center gap-3 px-5 py-4 rounded-2xl text-sm font-bold shadow-2xl
              ${toast.type==="error"
                ? "text-red-300"  : "text-emerald-300"}`}
            style={{
              background: toast.type==="error" ? "#1a050518" : "#05180b18",
              backdropFilter: "blur(20px)",
              border: toast.type==="error" ? "1px solid #ef444430" : "1px solid #10b98130",
              boxShadow: toast.type==="error" ? "0 8px 40px #ef44440d" : "0 8px 40px #10b9810d",
            }}
          >
            <span className="text-xl flex-shrink-0">{toast.type==="error"?"⚠️":"✅"}</span>
            <span className="flex-1 leading-snug">{toast.msg}</span>
            <motion.button whileTap={{ scale:0.85 }} onClick={()=>setToast(null)}
              className="opacity-40 hover:opacity-90 text-lg transition-opacity flex-shrink-0">✕</motion.button>
          </motion.div>
        )}
      </AnimatePresence>

      {/* ── STICKY HEADER ────────────────────────────────── */}
      <header className="sticky top-0 z-40 bg-[#06080f]/95 backdrop-blur-2xl"
        style={{ borderBottom: "1px solid #1e2235" }}>
        <div className="max-w-screen-xl mx-auto px-4 lg:px-8 py-4 flex items-center gap-4">

          {/* Animated hammer icon */}
          <motion.div
            animate={{ rotate: [0, -15, 5, -8, 0], scale: [1, 1.1, 1] }}
            transition={{ duration: 3.5, repeat: Infinity, repeatDelay: 2, ease: "easeInOut" }}
            className="w-11 h-11 rounded-2xl flex items-center justify-center text-xl flex-shrink-0"
            style={{ background: "#f59e0b15", border: "1px solid #f59e0b25", boxShadow: "0 0 20px #f59e0b10" }}
          >🔨</motion.div>

          <div className="flex-1 min-w-0">
            <h1 className="text-white font-black text-xl leading-none tracking-tight"
              style={{ fontFamily: "'Syne', sans-serif" }}>FabriCore Order</h1>
            <p className="text-[#2e3248] text-xs mt-0.5 font-medium">Naya order banao</p>
          </div>
          ...
          <div className="flex items-center gap-1">
            {STEPS.map((s, i) => (
              <React.Fragment key={s.n}>
                <StepPill
                  number={s.n} label={s.l}
                  state={step > s.n ? "done" : step === s.n ? "active" : "pending"}
                  onClick={() => {
                    if (s.n < step) goTo(s.n);
                    if (s.n === 2 && canS2) goTo(2);
                    if (s.n === 3 && canS2 && canS3) goTo(3);
                  }}
                />
      ...
      {/* ── PAGE BODY ────────────────────────────────────── */}
      <div className="relative z-10 max-w-screen-xl mx-auto px-4 lg:px-8 py-5 pb-24">
        ...
        {/* ══════════════════════════════════════════════════
            LAYOUT:
            Step 1 → centered single col (max-w-xl)
            Step 2+ → desktop 2-col [380px sidebar | content]
        ══════════════════════════════════════════════════ */}
              ...
              <motion.div key="s1" custom={dir} variants={sv} initial="enter" animate="center" exit="exit"
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                    {ORDER_TYPES.map(type => (
                      <OrderTypeCard key={type} type={type}
                        selected={customer.orderType === type}
                        onClick={() => setField("orderType", type)} />
                    ))}
                  </div>
                </div>
                ...
                <AnimatePresence>
                  {customer.orderType && (
                    ...
                    >
                      <ForgeField label="Customer Name" icon="👤">
                        <input value={customer.customerName} onChange={e=>setField("customerName",e.target.value)}
                          placeholder="Roopa Kumari" autoComplete="off" className={inp} />
                      </ForgeField>
                      <ForgeField label="Phone Number" icon="📞">
                        <input type="tel" inputMode="tel" value={customer.phone}
                          onChange={e=>setField("phone",e.target.value)} placeholder="8260519735" className={inp} />
                      </ForgeField>
                      <ForgeField label="Address" icon="📍">
                        <textarea rows={2} value={customer.address} onChange={e=>setField("address",e.target.value)}
                          placeholder="Jaltanda, Ranchi..." className={`${inp} resize-none`} />
                      </ForgeField>
                      <ForgeField label="Order Date" icon="📅">
                        <input type="date" value={customer.date} onChange={e=>setField("date",e.target.value)} className={inp} />
                      </ForgeField>
                    </motion.div>
                  )}
                </AnimatePresence>
                ...
                <ShimmerBtn
                  active={canS2}
                  onClick={() => canS2 ? goTo(2) : showToast("Type, naam aur phone required hai","error")}
        ...
        {step >= 2 && (
          <div className="lg:grid lg:grid-cols-[360px_1fr] xl:grid-cols-[400px_1fr] lg:gap-7 lg:items-start">
            ...
            {/* ── RIGHT: Step content ── */}
            <div>
              {/* Mobile: compact customer summary strip */}
              <div className="lg:hidden mb-4">
                <div className="flex items-center gap-3 rounded-2xl px-4 py-3"
                  style={{ background: "#0c0e1a", border: "1px solid #f59e0b25" }}>
                  ...
                  <motion.button whileTap={{ scale:0.88 }} onClick={() => goTo(1)}
                    className="text-[#3d4260] hover:text-amber-400 text-xs font-bold transition-colors flex-shrink-0">
                    Edit ✏️
                  </motion.button>
                </div>
              </div>
              ...
              <AnimatePresence mode="wait" custom={dir}>
                ...
                {step === 2 && (
                  <motion.div key="s2" custom={dir} variants={sv} initial="enter" animate="center" exit="exit"
                    transition={{ type:"spring", damping:32, stiffness:300 }} className="space-y-4">

                    <AnimatePresence initial={false}>
                      {orders.map((order, i) => (
                        <OrderCard key={i} order={order} index={i} orderType={customer.orderType}
                          onChange={(f,v) => setOF(i,f,v)} onRemove={() => remOrder(i)} />
                      ))}
                    </AnimatePresence>

                    {/* Add more */}
                    <motion.button type="button" whileTap={{ scale:0.97 }} whileHover={{ borderColor: "#f59e0b40" }}
                      onClick={addOrder}
                    ...
                    <div className="flex gap-3 pt-1">
                      <motion.button whileTap={{ scale:0.9 }} onClick={() => goTo(1)}
                        className="w-14 h-14 flex-shrink-0 flex items-center justify-center rounded-2xl text-white text-xl transition-colors"
                        style={{ background: "#0c0e1a", border: "1px solid #1e2235" }}
                      >←</motion.button>
                      <ShimmerBtn active={canS3}
                        onClick={() => canS3 ? goTo(3) : showToast("Har order ka detail bharo","error")}
                ...
                {step === 3 && (
                  <motion.div key="s3" custom={dir} variants={sv} initial="enter" animate="center" exit="exit"
                        ...
                        >
                          <div className="flex items-center gap-2 mb-2">
                            <span className="w-6 h-6 rounded-lg flex items-center justify-center text-xs font-black"
                              style={{ background: "#1e2235", color: "#4a5580" }}>{i+1}</span>
                          ...
                          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs pl-8">
                            ...
                            {o.extraCharge>0 && <span className="text-[#4a5580]">➕ ₹{o.extraCharge}</span>}
                    ...
                    <div className="grid grid-cols-2 gap-2.5">
                      {[
                        { label:"✏️ Customer Edit", action:()=>goTo(1) },
                        { label:"✏️ Orders Edit",   action:()=>goTo(2) },
                      ].map(b => (
                        <motion.button key={b.label} whileTap={{ scale:0.95 }} onClick={b.action}
                    ...
                    <button onClick={() => goTo(2)}
                      className="w-full py-3 text-[#2e3248] text-sm font-semibold hover:text-[#4a5580] transition-colors">
                      ← Wapis jao
                    </button>
                  </motion.div>
                )}
