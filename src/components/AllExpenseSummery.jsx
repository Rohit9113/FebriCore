...
const getEmpStats = (emp) => {
  ...
  const totalEarned = entries.reduce((sum, [date, v]) => {
    ...
    if (status === "present" || status === "auto-present") {
      return sum + getSalaryForDate(date, emp);
    }
    return sum;
  }, 0);
...
function SuperAdminPinModal({ onSuccess, onClose }) {
  ...
  const tryPin = (val) => {
    if (val === SUPER_ADMIN_PIN) { onSuccess(); }
...
export default function AllExpenseSummary() {
  ...
  const totalSalary = employees.reduce((s, emp) => {
    return s + Object.entries(emp.attendance || {}).reduce((sum, [date, v]) => {
      ...
      return (st === "present" || st === "auto-present")
        ? sum + getSalaryForDate(date, emp)
        : sum;
    }, 0);
  }, 0);
      ...
      <div className={`bg-[#1a1d27] border rounded-2xl overflow-hidden ${isSuperAdmin ? "border-red-500/30" : "border-white/10"}`}>
        <div className="flex justify-between items-center px-5 py-4 border-b border-white/10">
          <h2 className="font-semibold text-white flex items-center gap-2">
            Expense List <span className="text-gray-500 text-sm font-normal">({filtered.length} entries)</span>
                ...
                return (
                  <tr key={date} className={`border-t border-white/5 transition-colors ${isPresent?"bg-green-500/[0.03]":"bg-red-500/[0.03]"} ${isToday?"ring-1 ring-inset ring-amber-400/20":""} ${isSuperAdmin&&!isToday?"hover:bg-red-500/[0.04]":""}`}>
                    ...
                    <td className="px-5 py-3 text-right font-semibold">
                      {isPresent ? <span className="text-green-400">₹ {getSalaryForDate(date, employee).toLocaleString()}</span> : <span className="text-gray-500">—</span>}
                    </td>
                    <td className="px-5 py-3 text-center">
                      {canEdit ? (
                        <div className="flex gap-1 justify-center">
...
function SalaryDetailSummary({ employee, filterMonth, onUpdate, toast }) {
  ...
  const handleMarkPaid = async () => {
    ...
    try {
      ...
      const newPayment = { amount:payAmountNum, paidOn:TODAY, note:payNote||`₹${payAmountNum.toLocaleString("en-IN")} payment`, dates:[] };
  ...
  return (
    <div className="p-5 space-y-5">
      ...
      {(employee.salaryPayments||[]).length>0 && (
        <div>
          <h4 className="text-gray-300 font-semibold mb-3 flex items-center gap-2">📋 Payment History <span className="text-xs text-gray-500 font-normal">({employee.salaryPayments.length} transactions)</span></h4>
          <div className="space-y-2">
            {(()=>{let runningPaid=0; return [...(employee.salaryPayments||[])].reverse().map((pay,i)=>{runningPaid+=pay.amount||0; const remaining=Math.max(0,totalEarned-runningPaid); return (
...
function EmployeeDetailDrawer({ employee, onClose, onUpdate, onToggleActive, toast }) {
  ...
  const saveIncrement = async () => {
    ...
    try {
      ...
      setIncForm({newSalary:"",reason:"",effectiveDate:TODAY}); setShowIncrementForm(false); toast(data.message);
  ...
  const handleSave = async () => {
    ...
    try {
      if (type==="salary") {
        ...
        await onSave({ name:form.name, phone:form.phone, address:form.address||"", joiningDate:form.joiningDate||TODAY, perDaySalary:Number(form.perDaySalary) });
      } else {
