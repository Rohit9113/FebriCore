"use client";
// src/components/EmployeeSalaryManager.jsx
...
function OvertimeModal({ employee, date, onConfirm, onClose }) {
  ...
  const otPay = mode === "amount" ? (Number(customAmt) || 0) : (Number(hours) || 0) * hourlyRate * 1.5;
      ...
      <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }}
        ...
        <div onClick={(e) => e.stopPropagation()} className="bg-[#0f1117] border border-purple-500/40 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
          ...
          <div className="flex bg-white/5 rounded-xl overflow-hidden border border-white/10 mb-5">
            <button onClick={() => setMode("amount")} className={`flex-1 py-2.5 text-sm font-semibold transition ${mode === "amount" ? "bg-purple-500 text-white" : "text-gray-400 hover:text-white"}`}>₹ Custom Amount</button>
            <button onClick={() => setMode("hours")} className={`flex-1 py-2.5 text-sm font-semibold transition ${mode === "hours" ? "bg-purple-500 text-white" : "text-gray-400 hover:text-white"}`}>🕐 By Hours</button>
          </div>
          <AnimatePresence>
            {mode === "amount" && (
              <motion.div key="amount" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="mb-5">
                ...
                <div className="relative">
                  ...
                  <input type="text" inputMode="decimal" value={customAmt} onChange={(e) => setCustomAmt(e.target.value.replace(/[^0-9.]/g, ""))}
                    placeholder="e.g. 150" autoFocus
          ...
          {otPay > 0 && (
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="flex justify-between text-sm"><span className="text-gray-400">Base (Full Day)</span><span className="text-white font-semibold">₹ {perDaySalary.toLocaleString("en-IN")}</span></div>
              <div className="flex justify-between text-sm"><span className="text-gray-400">Overtime</span><span className="text-purple-400 font-semibold">+ ₹ {Math.round(otPay).toLocaleString("en-IN")}</span></div>
...
export default function EmployeeSalaryManager() {
  ...
  useEffect(() => { if (employees.length === 0) return; autoMarkToday(); }, [employees.length]);
  ...
  return (
    <div style={{ fontFamily: "'DM Sans','Segoe UI',sans-serif" }} className="text-white p-4 md:p-5 pb-6">
      ...
      <div className="mb-5 flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Employee <span className="text-amber-400">Manager</span></h1>
          <p className="text-gray-400 text-sm mt-0.5">Salary aur attendance track karo</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setShowAdd(true)} className="px-5 py-2 rounded-full text-sm font-semibold bg-blue-500 hover:bg-blue-400 transition shadow-lg shadow-blue-500/30">
            + Add Employee
          </button>
        </div>
      </div>
      ...
      <SalarySection employees={employees} setEmployees={setEmployees} totalDue={totalDue}
        ...
        fetchEmployees={fetchEmployees} loading={loading} />

      <AnimatePresence>
        {showAdd && (
          <Modal onClose={() => setShowAdd(false)}>
            <AddEmployeeForm onClose={() => setShowAdd(false)} onSave={handleAddEmployee} employeeCount={employees.length} />
          </Modal>
        )}
      </AnimatePresence>
...
function AttendanceTable({ employee, filteredEntries, onToggle, readonly = false, isSuperAdmin = false }) {
  ...
  const paginatedEntries = filteredEntries.slice((currentPage - 1) * ATTENDANCE_PER_PAGE, currentPage * ATTENDANCE_PER_PAGE);
  ...
  const rowVariants = {
    ...
    visible: (i) => ({ opacity: 1, filter: "blur(0px)", y: 0, transition: { delay: i * 0.05, duration: 0.35, ease: "easeOut" } }),
      ...
      {filteredEntries.length === 0 ? (
        ...
        <>
              ...
              <tbody>
                <AnimatePresence>
                  {paginatedEntries.map(([date, entry], i) => {
                    ...
                    return (
                      <motion.tr key={`${date}-${currentPage}`} custom={i} variants={rowVariants} initial="hidden" animate="visible" exit="exit"
          ...
          <div className="px-5 pb-4 pt-3 border-t border-white/5 flex items-center justify-between flex-wrap gap-2">
            <p className="text-xs text-gray-500">Showing {Math.min((currentPage - 1) * ATTENDANCE_PER_PAGE + 1, filteredEntries.length)}–{Math.min(currentPage * ATTENDANCE_PER_PAGE, filteredEntries.length)} of {filteredEntries.length} records</p>
            <Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} />
          </div>
        </>
      )}
...
function DayList({ title, count, color, icon, dates, employee, noSalary = false }) {
  const cm = {
    green: { h: "text-green-400", d: "bg-green-400", r: "bg-green-500/8 border-green-500/15", s: "text-green-400" },
    yellow: { h: "text-yellow-400", d: "bg-yellow-400", r: "bg-yellow-500/8 border-yellow-500/15", s: "text-yellow-400" },
    red: { h: "text-red-400", d: "bg-red-400", r: "bg-red-500/8 border-red-500/15", s: "text-gray-600" },
  };
...
// ─── Add Employee Form ────────────────────────────────────────────
function AddEmployeeForm({ onClose, onSave, employeeCount }) {
  const [form, setForm] = useState({ joiningDate: TODAY });
  ...
  const handleSave = async () => {
    if (!form.name || !form.phone || !form.perDaySalary) { alert("Name, Phone aur Salary required hai"); return; }
    setSaving(true);
    try {
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold mb-5 text-white">👤 New Employee</h2>
      <div className="space-y-4">
