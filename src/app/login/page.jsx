...
function GridBackground() {
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden">
      ...
      <motion.div animate={{ scale: [1, 1.15, 1], opacity: [0.1, 0.18, 0.1] }}
      ...
      {[{ x:"8%",y:"15%",size:200,dur:9 },{ x:"82%",y:"65%",size:240,dur:12 },{ x:"55%",y:"8%",size:160,dur:8 }]
        .map((o, i) => (
          <motion.div key={i} animate={{ y:[0,-28,0], x:[0,14,0] }}
...
function TiltCard({ children }) {
  ...
  const onMove  = (e) => {
    ...
    setRot({ x: ((e.clientY - r.top) / r.height - 0.5) * -5, y: ((e.clientX - r.left) / r.width - 0.5) * 5 });
  };
...
export default function LoginPage() {
  ...
  useEffect(() => { if (admin) router.replace("/dashboard"); }, [admin, router]);
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      try {
        ...
        // 404 = admin nahi mila → employee try karo
      }
      ...
      try {
        ...
        if (!data?.success) {
          setError(data?.error || "Login failed — dobara try karo");
          setShakeKey(k => k + 1);
          return;
        }
      ...
      setError(!err.response ? "Internet ya server issue — dobara try karo" : "Server error — thodi der baad try karo");
      setShakeKey(k => k + 1);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-[#0a0c14] flex items-center justify-center px-4 py-8"
      ...
      <div className="w-full max-w-sm relative z-10">
        ...
        <motion.div initial={{ opacity:0, y:-24 }} animate={{ opacity:1, y:0 }}
          ...
          <p className="text-[#45506e] text-sm mt-1.5">Apne account mein login karo</p>
        </motion.div>
        ...
        <TiltCard>
          <motion.div key={shakeKey}
            ...
            <form onSubmit={handleLogin} className="relative space-y-5">
              ...
              {/* Phone */}
              <motion.div initial={{ opacity:0, x:-14 }} animate={{ opacity:1, x:0 }} transition={{ delay:0.38, duration:0.45 }}>
                <label className="block text-[10px] font-black uppercase tracking-[0.18em] text-[#3d4870] mb-2.5">Phone Number</label>
              ...
              {/* Password */}
              <motion.div initial={{ opacity:0, x:-14 }} animate={{ opacity:1, x:0 }} transition={{ delay:0.48, duration:0.45 }}>
                <label className="block text-[10px] font-black uppercase tracking-[0.18em] text-[#3d4870] mb-2.5">Password</label>
              ...
              <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }}
                transition={{ delay:0.58, duration:0.4 }} className="pt-1">
